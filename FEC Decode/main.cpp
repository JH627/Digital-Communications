...
void fec_decode(uint8_t* dataPtr, uint8_t* encoded) {
    uint32_t tmp = 0;
	tmp = (dataPtr[2] << 16) | (dataPtr[3] << 8) | dataPtr[4]; // 받아온 data를 한곳에 저장

    int zero_count = 0; // bit 중 0의 총 개수를 저장
    for (int i = 7; i >= 0; i--) {
        zero_count = 0;
        for (int j = 0; j < 3; j++) {
        	if (((tmp & (1 << (i * 3 + j))) >> (i * 3 + j)) == 0) {
        		zero_count++;
            }
        }
        if (zero_count >= 2) {
            encoded[0] = (encoded[0] << 1); // 끝에 0 추가
        }
        else {
            encoded[0] = (encoded[0] << 1) | 0x01; // 끝에 1 추가
        }
	}
}
...
void arqMain_processInputWord(void) {
    char c = pc.getc();
    if (main_state == MAINSTATE_IDLE && !arqEvent_checkEventFlag(arqEvent_dataToSend)) {
        ...
        switch (main_state) {
            ...
            case MAINSTATE_IDLE: //IDLE state description
                ...
                if (arqEvent_checkEventFlag(arqEvent_dataRcvd)) { //if data reception event happens
                     ...
                     **/
                    uint8_t decoded = 0;
                    fec_decode(dataPtr, &decoded);

                    /**
                     *  수정됨 argMsg_getWord(dataPtr) -> decoded
                     */
                    pc.printf("\n -------------------------------------------------\nRCVD from %i : %c (length:%i, seq:%i)\n -------------------------------------------------\n", 
                                srcId, decoded, size, arqMsg_getSeq(dataPtr));
            ...
            case MAINSTATE_TX: //TX state description
                ...
                if (arqEvent_checkEventFlag(arqEvent_ackTxDone)) { //data TX finished
                    if (arqTimer_getTimerStatus() == 1 || arqEvent_checkEventFlag(arqEvent_arqTimeout)) {
